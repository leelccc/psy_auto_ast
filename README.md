...
- **验证**：移动端 TypeScript 检查及 99 项测试通过；手机号后端集成测试 5/5 通过；Alembic 确认单一 head `c2d4e8f1a0b2`。全后端旧套件 72 项中 63 项通过，9 项因旧邮箱注册测试未提供验证码及既有演示数据断言漂移失败，与本批手机号流程无关。
- **上线状态**：本批未部署、未打包。真实短信上线前仍需配置阿里云 AK/SK、审核通过的短信签名和注册/登录/重置密码三个模板 ID。
