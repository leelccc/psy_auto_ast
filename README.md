...
### 2026-09-07 · 转写/纪要升级为历程级，咨询记录直接消费录音纪要

- **后端（模型与迁移）**：转写 `recording_transcripts` 与纪要 `recording_summaries` 新增 `session_id` 列、外键、索引及唯一约束（每历程各最多一份），迁移 `f6a8b0c2d4e7_make_transcript_summary_session_scoped`；AI 任务 `target_type` 由 `recording` 改为 `session`，同历程一组录音共享处理状态，开始转写后整组锁定。
- **录音处理**：未归档入咨询历程的录音不再允许发起转写（返回 409），必须先选 1–5 条未归档录音归入历程并调序，再统一转写合并为历程级完整转写。
- **咨询记录（报告）**：删除原「只填系统事实、专业段落留白」的骨架草稿，改为由真实 LLM 直接生成完整草稿，并校验每个指定段落非空，否则报错（生产未配模型返回 503）；`list_sources` 不再列出完整转写，仅列历程级录音纪要，非 `case_report` 类型生成/重生成前强制要求存在本次历程录音纪要且其必须作为生成资料。
- **前端**：报告来源文案「录音」统一改为「录音纪要」，`recordingService` 的 transcript/summary 类型补充 `sessionId` 字段；移动端 BUILD_TAG 升至 `0907-1`、Android `versionCode` 升 2 并更新自适应图标。
- **权限与清理**：transcript/summary 访问权限优先按 `session_id` 判定；删除历程时按 `session.id` 一并级联清理其转写、纪要、报告与敏感资源登记。

### 2026-09-06 · 转写可读性与正式 App 图标

- 百炼文件转写结果会把同一说话人连续、间隔不超过 2 秒的句子合并为一次发言；换人后再形成下一段，避免逐句碎片化。
- 章节继续由后端限制为最多 8 个；录音详情页可点选章节，并且只预览该章节内最多 3 次发言、每次最多 4 行，完整内容进入专门的转写校对页查看。
- 采用“咨询档案 + 洞察”第 4 版图标，完成正视化、不透明底色和手机蒙版安全区调整，并更新 iOS/Android 共用图标资源。

### 2026-09-06 · 历程状态修复与智能督导对话化

- 咨询历程的录音状态改由后端真实录音记录派生，进入录音资料时再刷新一次列表；兼容旧录音 `session_index` 为空的数据，避免页面显示“未添加”却被后端判定已开始转写。
- 咨询记录保存正式版后立即更新当前报告、切换正式版内容并进入只读状态，不再停留在“保存为正式版”按钮。
- 智能督导改为对话优先：进入即显示督导助手与对话，输入栏左侧 `+` 选择讨论对象，历史会话收进右上角；明显超出心理咨询督导范围的问题会被拒绝并引导回专业讨论。

### 2026-09-06 · 录音整理页交互与 Web 样式修复

- Web 端录音片段改为卡片内嵌紧凑播放器，移除重复的播放器卡片、标题和时长信息；“等待选择”恢复为固定尺寸状态标签。
- 待处理的录音整理页新增“从未归档录音中添加”，可继续选择录音记录中的 1–5 条独立录音；录制和上传入口仍只保留在最外层“录音记录”。
- 后端允许在尚未开始转写时向同一历程追加录音，并继续强制校验每条录音仅一个文件、总数不超过 5、总大小不超过 300MB；转写开始后保持永久锁定。

### 2026-09-05 · 多片段录音与手动转写

- 一次咨询 / 督导 / 受督历程支持最多 5 个录音片段，单文件不超过 200MB、合计不超过 300MB、单段不超过 2 小时。
- 收集中可继续录制、上传、播放、拖动或按钮调序及删除；点击“完成并转写”确认后永久锁定录音内容。
- 后端并行识别全部片段，按锁定顺序合并时间轴和全文，再统一生成纪要；失败时重新转写全部片段。
- 每个源文件从各自上传时间起独立保留 14 天；删除整个历程会级联删除片段文件、转写、纪要和报告。
- 已部署 Web、后端与迁移 `d3e5f7a9b1c4`；上线前备份数据库、MinIO、后端及 Web，并使用 `backend/scripts/cleanup_test_business_data.sql` 清除测试业务数据。用户账户、认证绑定、登录会话及系统配置均已保留。
- 生产环境用两段有效中文 M4A 完成真实转写验收：两段均识别、按序合并并成功生成总纪要；320px Web 视口下片段卡、调序和锁定弹窗无溢出。验收数据随后再次清空。

### 2026-09-05 · 手机号认证完整闭环（已部署，待短信资质启用）

- **后端**：新增手机号验证码、注册、密码登录、验证码登录（首次自动注册）和重置密码接口；手机号与邮箱账号保持独立。`users.email` 改为可空并新增唯一手机号，新增 `phone_verification_codes` 表及 Alembic 迁移。
- **短信通道**：接入阿里云短信 SDK，验证码仍遵循 60 秒冷却、10 分钟有效、最多 5 次尝试且只存哈希；开发环境未配置短信时返回 `dev_code`，生产环境配置不全时拒绝发送。
- **移动端**：登录页新增“邮箱 / 手机号”切换，手机号覆盖密码登录、验证码登录、注册和忘记密码；“我的”页兼容显示手机号及实际登录方式，管理后台用户列表兼容手机号账号。
- **当前启用状态**：Web 入口与后端能力均已保留并部署；生产环境未配置短信密钥时，发码接口明确返回“短信服务暂不可用”，且不会写入验证码或触发 60 秒冷却。资质申请完成后配置短信环境变量并重启后端即可启用。
- **窄屏适配**：验证码输入框与发码按钮在小于 360px 的视口下改为纵向全宽排列，避免按钮越出登录卡片。
- **验证**：移动端 TypeScript 检查及 99 项测试通过；手机号后端集成测试 6/6 通过；Alembic 确认单一 head `c2d4e8f1a0b2`。全后端旧套件 72 项中 63 项通过，9 项因旧邮箱注册测试未提供验证码及既有演示数据断言漂移失败，与本批手机号流程无关。
- **上线状态**：Web 与后端已部署，数据库迁移已执行；Android 无需因本次 Web 布局修复重新打包。真实短信启用前仍需配置阿里云 AK/SK、审核通过的短信签名和注册/登录/重置密码三个模板 ID。
